...
async function main() {
  ...
  // Programmer quelques cours pour la 1ère AM A
  const coursExemples = [
    // Lundi
  ...
  for (const coursData of coursExemples) {
    await prisma.cours.create({
      data: {
        ...
        enseignantId: enseignants[coursData.enseignantIndex].enseignant.id,
  ...
  console.log('📋 Génération dun bulletin exemple...');
